...
- 感知机概念：
  &emsp;&emsp; 由两层神经元组成，输入层接收外界信号后传递给输出层，输出层是 M-P 神经元（阈值逻辑单元）
- 感知机学习规则：
...
- BP算法基本流程：  
  ...
  **过程：**  
  ...
  (6) &emsp;&emsp;根据$e_h=b_h(1-b_h)\sum_{j=1}^l w_{hj} g_j$隐藏层神经元的梯度项$e_h$；  
  (7) &emsp;&emsp;更新连接权$w_{hj},v_{ih}$与阈值$\theta_j,\gamma_h$；  
  (8) &emsp;**end for**  
  (9) **until** 达到停止条件  
  **输出：** 连接权与阈值确定的多层前馈神经网络
- BP算法更新规则：  
  &emsp;&emsp;基于每个样本的预测值与真实类标的均方误差来进行权值调节，即每次更新只针对单个样例。其最终目标是要最小化整个训练集$D$上的累积误差，即：$E=\frac{1}{m} \sum_{k=1}^m E_k$
