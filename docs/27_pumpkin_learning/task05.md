...
- 间隔：两个异类支持向量超平面的距离之和 $\gamma=\frac{2}{\|w\|}$
- 支持向量机（SVM）基本型：

  &emsp;&emsp; 找到最大间隔的划分超平面，需要求解参数 $w$ 和 $b$ 使得 $\gamma$ 最大，目标函数如下：$$\begin{array}{l} \min \limits_{w,b} & \frac{1}{2}\|w\|^2 \\

\text { s.t. } & y_{i}(w^T x_i+b ) \geqslant 1, \quad i=1,2, \ldots, m \end{array}$$

## 2 对偶问题

- 拉格朗日函数：$L(w, b, \alpha)=\frac{1}{2}\|w\|^2 + \sum_{i=1}^m \alpha_i\left(1y_i (w^T x_i+b ) \right)$
- 对偶问题：$$\begin{array}{ll} \max_{\alpha} & \sum_{i=1}^m \alpha_i-\frac{1}{2} \sum_{i=1}^m \sum_{j=1}^m \alpha_i \alpha_j y_i y_j x_i^T x_j \\
{\text { s.t. }} & {\sum_{i=1}^m \alpha_i y_i=0} \\
{} & {\alpha_i \geqslant 0, i=1,\ldots, m}
\end{array}$$

- KKT 条件：$$\left \{ \begin{array}{ll}
\alpha_i \geqslant 0 \\
...
- 损失函数：
  1. hinge 损失：$\ell_{hinge}(z)=\max(0,1-z)$
