...
$$
\text{Err}(\hat{f})=\text{Bias}^2 (\hat{f})+\text{Var}(\hat{f})+\sigma_{\varepsilon}^2
$$其中$\varepsilon \sim N(0,\sigma_{\varepsilon})$时噪声，潜在模型为$Y=f(x) + \varepsilon$，估计模型为$\hat{f}(X)$
...
- 线性模型一般形式：$f(x^{(k)}) = \sum_{i=1}^m w_i x_i^{(k)} + b$
- 优化目标：$(w^*,b^*) =\mathop{\arg\min} \limits_{(w,b)} \sum_{k = 1}^n(w^T x^{(k)}+b-y^{(k)})^2$
...
- 常见方法：K-Means 聚类、均值漂移聚类、基于密度的聚类
- K-Means 聚类算法步骤：
  1. 选取 $K$ 个对象作为初始聚类中心
  2. 计算样本数据与聚类中心的欧式距离，将数据按照聚类中心进行分类
  3. 更新聚类中心，计算每个类别所有对象的均值，将其作为聚类中心，计算目标函数的值
  4. 判断聚类中心和目标函数的值是否发生变化，没有变化则输出聚类结果，否则返回步骤 2
