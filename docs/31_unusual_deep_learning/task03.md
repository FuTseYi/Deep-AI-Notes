...
- 神经元 M-P 模型：接收 n 个神经元的输入信号，经过加权求和，结果与阈值 $\theta$ 比较，经过激活函数得到输出
...
- 训练步骤：
  ...
  3. 迭代调整参数，直到误差为 0 或者小于 $\varepsilon$
...
&emsp;&emsp; 本次任务，主要介绍了前馈神经网络，神经元模型也称为 M-P 模型，后提出感知器，采用有监督学习的方式进行模型训练，能够自动确定参数，主要学习方法是误差修正；之后 BP 算法，也称为误差反向传播算法，通过计算实际输出与期望输出的误差，把误差进行反向传递，采用梯度下降法调整各层的连接权重减小误差。该算法主要存在一些难点：参数过多，无法解决非凸优化问题，存在梯度消失问题，参数的解释性不强。
