...
当参数的微分为 0 的时候，梯度下降无法继续更新参数，导致训练损失误差无法下降
...
$H$ 表示是 $L(\theta')$ 的二次微分（即 Hession 矩阵）：

$$
H_ij = \frac{\partial^2}{\partial \theta_i \partial \theta_j} L(\theta')
$$

2. 如果是 `critical point`（临界点），则 $(\theta - \theta')^T g = 0$，会出现以下 3 种情况：
...
1. 当在临界点时，即 $L(\theta) \approx L(\theta')+ \frac{1}{2}(\theta - \theta')^T H (\theta - \theta')$
...
（1）当 batch size 很大时，并不需要很长的时间计算梯度（除非 batch size 太大了）
（2）很小的 batch 在一个 epoch 上需要耗费更多的时间（遍历所有的数据）
（3）小的 batch 会使得模型有更好的效果
...
其中 $g_i^t = \frac{\partial L}{ \partial \theta_i} |_{\theta=\theta^t}$
...
- Root Mean Square：$\sigma_i^t = \sqrt{\frac{1}{t+1} \sum_{i=0}^t (g_i^t)^2}$
...
其中，如果采用 MSE（Mean Square Error），则 $e = \sum_i (\hat{y}_i - y'_i)^2$

如果采用交叉熵（Cross-entropy），则 $e = -\sum_i \hat{y}_i \ln y'_i$，这个更常用。
...
2. 使用 Batch 能加快梯度的计算，添加动量能更好的越过局部最小值或鞍点
3. 利用 RMS 和 RMSProp 自动调整学习率，并使用学习率调度，加快梯度的下降速度
4. 使用交叉熵能更好的计算损失
