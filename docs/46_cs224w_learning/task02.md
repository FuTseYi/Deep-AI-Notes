...
- 不同的任务类型：包括 Graph 层、Node 层、Community(subgraph) 层、Edge 层
...
- Interactions（关系）：links、edges 表示为 $E$
- System：network、graph 表示为 $G(N,E)$
...
- 无向图：平均度为 $\bar{k} = \langle k \rangle = \frac{1}{N} \sum_{i=1}^N k_i = \frac{2E}{N}$
- 有向图：平均度为 $\bar{k} = \frac{E}{N}$

### 2.5 邻接矩阵 Adjacency Matrix

- 无向图：
    - 邻接矩阵是对称阵 $A_{ij} = A_{ji}$，主对角线为 0，$A_{ii} = 0$
    - 连接总数：$L = \frac{1}{2} \sum_{i=1}^N k_i = \frac{1}{2} \sum_{ij}^N A_{ij}$
    - 沿行或列求和均可：$k_i = \sum_{j=1}^N A_{ij}$，$k_j = \sum_{i=1}^N A_{ij}$
- 有向图
    - 邻接矩阵是非对称矩阵，$A_{ij} \neq A_{ji}$，主对角线为 0，$A_{ii} = 0$
    - 连接总数：$L = \sum_{i=1}^N k_i^{in} = \sum_{j=1}^N k_j^{out} = \sum_{ij}^N A_{ij}$
...
- In-component：节点指向 SCC
- Out-component：节点指出 SCC
...
公式：$c_v = \frac{1}{\lambda} \sum_{u \in N(v)} c_u$，$v$ 节点邻居节点的重要度求和
...
- 衡量节点有多抱团
- 公式：$e_v = \frac{\#(\text{edges among neighboring nodes})}{
\left ( \begin{array}{c}
k_v \\
2
...
交并比（Jaccard's coefficient）：$\frac{|N(v_1) \cap N(v_2)|}{|N(v_1) \cup N(v_2)|}$

共同好友是不是社牛（Adamic-Adar index）：$\sum_{u \in N(v_1) \cap N(v_2)} \frac{1}{\log (k_u)}$
...
$S_{v_{1}v_{2}} = \sum_{l=1}^{\infty} \beta^l A_{v_{1}v_{2}}^l$，其中 $\beta$ 为折减系数，$0 < \beta < 1$
...
- 区别：计算全图的 Graphlet 个数，而非特定节点领域 Graphlet 个数
- 公式：$(f_G)_i = \#(g_i \subseteq G) \text{ for } i = 1,2,\cdots, n_k$，表示第 $i$ 个 Graphlet 在全图中的个数

**Graphlet Kernel：**

- 两个图的 graphlet kernel：$K(G, G') = f_G^T f_{G'} $，两个图的 Graphlet Count Vector 数量积
- 计算上述公式太过复杂，该问题是 NP-hard
